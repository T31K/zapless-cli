  ...
  program
    ...
    .action(async () => {
      ...
      if (sessionExists) {
        ...
        console.log(chalk.red("✘") + "  No session file — run: zapless auth login --token <token>");
        allGood = false;
      }
      ...
      if (session?.install_token) {
        ...
        console.log(chalk.red("✘") + "  Session file is corrupt — run: zapless auth login --token <token>");
        allGood = false;
      }

      // 3. Server reachable
      try {
        await axios.get(`${CONFIG.SERVER_URL}/api/zapless/health`, { timeout: 5000 });
        console.log(chalk.green("✔") + `  Server reachable (${CONFIG.SERVER_URL})`);
      } catch {
        try {
          // fallback: hit any known endpoint
          await axios.post(`${CONFIG.SERVER_URL}/api/zapless/auth`, {}, { timeout: 5000 });
        } catch (err: any) {
          if (err.response) {
      ...
      if (session?.install_token) {
        try {
          ...
          if (connected_apps.length > 0) {
            ...
            console.log(chalk.yellow("⚠") + "  No apps connected — visit zapless.app/dashboard");
          }
        } catch (err: any) {
          if (err.response?.status === 401) {
            console.log(chalk.red("✘") + "  Auth token invalid — re-run: zapless auth login --token <token>");
          } else {
